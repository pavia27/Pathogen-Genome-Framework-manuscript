...
PMIDs from 2023-2024 were downloded from LitCovid and the BioC xml for those papers that were in the PMC_OA subset were downloaded. Using regular expressions, the xml files were searched for terms related to SARS-CoV-2 variants of interest and expressions related to sequences and sequence repositories.
The specific SARS-CoV-2 variants we looked for were as follows: BA.2,  BA.2.86, JN.1, KP.2, KP.3, KP.3.1.1, LB.1, XEC, JN.1.7, and JN.1.18. 
...
First, we compared the total number of unique amino acid (AA) and nucleotide (NT) mutations between the two groups. Patients with missing immune status were excluded from this comparison. The boxplot shows that immunocompromised patients tend to have a higher mutation burden.
...
Models were fit on `subset_patient_coded.csv` (100 genomes with patient-level outcomes). Each specification uses `glm` (binomial) or `lm` (infection length) for inference (Wald p-values, OR or β, 95% CIs) and `caret` 5-fold cross-validation (`glm`/`lm`) for AUC, Brier, or RMSE. Genotype–phenotype associations significant (p < 0.05) in at least one outcome are exported to `Table1_*.csv` (not printed in this document).
...
# Cohort outcome summary (genome-level), overall and by study
fmt_inf_range <- function(x) {
  avail <- x > 0 & !is.na(x)
  if (!any(avail)) return("\u2014")
  paste0(min(x[avail]), "\u2013", max(x[avail]))
}

h_cohort <- h %>%
  left_join(filtered_df %>% distinct(genbank, study_id), by = "genbank") %>%
  mutate(inf_available = infection_length > 0 & !is.na(infection_length))

cohort_by_study <- h_cohort %>%
  group_by(study_id) %>%
  summarise(
    `N patients` = n_distinct(id),
    `N genomes` = n(),
    Mortality = sum(death == "Yes"),
    Hospitalization = sum(hospitalization == "Yes"),
    `Infection duration (data available)` = sum(inf_available),
    `Range (days)` = fmt_inf_range(infection_length),
    .groups = "drop"
  ) %>%
  rename(Study = study_id)

cohort_total <- tibble(
  Study = "Total (modeling cohort)",
  `N patients` = n_distinct(h_cohort$id),
  `N genomes` = nrow(h_cohort),
  Mortality = sum(h_cohort$death == "Yes"),
  Hospitalization = sum(h_cohort$hospitalization == "Yes"),
  `Infection duration (data available)` = sum(h_cohort$inf_available),
  `Range (days)` = fmt_inf_range(h_cohort$infection_length)
)

cohort_outcome_table <- bind_rows(cohort_by_study, cohort_total)
write.csv(cohort_outcome_table, "Table_modeling_cohort_outcomes.csv", row.names = FALSE)
